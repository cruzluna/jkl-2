...
print_message muted "  Paste into Codex to have it set up the hooks for you:"
print_message info "      Set up jkl status hooks for Codex by running \`jkl init hooks --tool codex --scope global --non-interactive\`, then remind me to run /hooks to review and trust them."
print_message muted "  Or run the CLI directly, then open /hooks in Codex to review and trust the command hooks:"
print_message info "      jkl init hooks --tool codex --scope global --non-interactive"
print_message muted "  Events: SessionStart/Stop = idle, UserPromptSubmit/PostToolUse = working, PermissionRequest = blocked."
print_message muted "  Note: Esc-interrupted turns do not fire Stop (openai/codex#22858); SessionStart resets to idle on start/resume/clear."
printf "\n"
